                ...
                <div className="container-4/5 mt-8 flex flex-col">
                    ...
                    </div>
                    {
                        riders.map((r, i) => {
                            ...
                            var bg = 'bg-red-500';
                            if((40< percentage && percentage<50 )|| percentage ===40) {bg='bg-red-300'; console.log(percentage);}
                            else if((50< percentage && percentage<70 )|| percentage ===50) {bg='bg-green-300'; console.log("a", percentage)}
                            else if((70< percentage && percentage<101 )|| percentage ===70) {bg='bg-green-500'; console.log("aa",percentage)}

                            return (
                                <div onClick={()=> navigate(`${r.id}`)} className="grid grid-cols-5 pt-4 pb-2 border-b-2 last:border-b-slate-50 hover:border-b-2 hover:border-gray-500 cursor-pointer hover:bg-white hover:translate-x-1 transition duration-100 ease-linear">
                                    <p className="pl-2">{r.email}</p>
                                    <p className="pl-2">{r.name}</p>
                                    <p className="pl-2">{r.vehicleType}</p>
                                    <p className="pl-2">{r.plate}</p>
